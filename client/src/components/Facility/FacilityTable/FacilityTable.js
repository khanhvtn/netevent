    ...
    <Toolbar
      ...
      {numSelected === 0 ? (
        ...
        <>
          ...
          <Button
            onClick={(e) => handleToggleDialogCreateAndUpdate(e, 'edit')}
...
const FacilityTable = ({
  ...
  const css = useStyles();
  const { facilities, isLoading, createSuccess, deleteSuccess, updateSuccess } =
    useSelector((state) => ({
        ...
        <Table
          ...
          <TableBody>
            {isLoading || createSuccess || updateSuccess || deleteSuccess ? (
              ...
              <>
                {stableSort(facilities, getComparator(order, orderBy)).map(
                  (row, index) => {
