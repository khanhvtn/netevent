            ...
            <Grid container direction="column">
              <Toolbar>
                ...
                <Button
                  ...
                  variant="contained"
                  color={state.isRecycleMode ? 'default' : 'secondary'}
                  endIcon={state.isRecycleMode ? <Close /> : <Delete />}>
                  {state.isRecycleMode ? 'Close' : 'Bin'}
                </Button>
                <Tooltip title="Filter" className={css.filterButton}>
                  <div>
                    <IconButton
                      disabled={isLoading}
                      color="inherit"
          ...
          <Grid
            ...
            {isLoading ? (
              Array.apply(null, { length: state.take }).map(
                (skeleton, index) => {
                  return <EventCard key={index} isLoading={isLoading} />;
                }
              )
            ) : events.length === 0 ? (
              <Grid
