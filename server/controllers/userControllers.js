...
const login = async (req, res, next) => {
  try {
    ...
    //check password
    const result = await bcrypt.compare(userReq.password, existedUser.password);
    if (!result) {
...
const filterUser = async (req, res, next) => {
  try {
    ...
    const createdMaxDate = await User.find().sort({ createdAt: -1 }).limit(1);
    const createdMinDate = await User.find().sort({ createdAt: 1 }).limit(1);
    const updatedMaxDate = await User.find().sort({ updatedAt: -1 }).limit(1);
    const updatedMinDate = await User.find().sort({ updatedAt: 1 }).limit(1);
